...
function InstructorLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      setIsLoading(true);
      try {
        // Add role specification for instructor login
        await login({ ...formData, role: 'instructor' });
        // AuthContext will handle redirection to /instructor
      } catch (err) {
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="bg-gray-950/50 backdrop-blur-xl rounded-2xl border border-gray-800/50 shadow-2xl p-8 transform hover:scale-[1.02] transition-all duration-500">
            ...
            <div className="text-center mb-8">
              <div className="w-16 h-16 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-2xl flex items-center justify-center mx-auto mb-4 transform hover:rotate-12 transition-all duration-500 hover:scale-110">
              ...
              <p className="text-gray-400">
                Inspire, train, and transform lives through fitness
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-gray-800/30 rounded-lg p-3 text-center">
                <FiUsers className="w-6 h-6 text-purple-400 mx-auto mb-1" />
                <p className="text-xs text-gray-400">Student Management</p>
              </div>
              <div className="bg-gray-800/30 rounded-lg p-3 text-center">
                <FiTarget className="w-6 h-6 text-indigo-400 mx-auto mb-1" />
                <p className="text-xs text-gray-400">Workout Plans</p>
              </div>
              <div className="bg-gray-800/30 rounded-lg p-3 text-center">
                <FiHeart className="w-6 h-6 text-purple-400 mx-auto mb-1" />
                <p className="text-xs text-gray-400">Progress Tracking</p>
              </div>
              <div className="bg-gray-800/30 rounded-lg p-3 text-center">
                <FiZap className="w-6 h-6 text-indigo-400 mx-auto mb-1" />
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Email Field */}
              <div className="group">
                <label className="block text-gray-300 mb-2 transition-colors group-focus-within:text-purple-400">
                  Email
                </label>
                <div className="relative">
                  <FiMail className="absolute left-4 top-3.5 text-gray-500 transition-colors group-focus-within:text-purple-400" />
                  <input
                    type="email"
                    name="email"
                    value={formData.email}
                    onChange={handleChange}
                    className="w-full bg-gray-900/50 text-white rounded-lg pl-12 pr-4 py-3 border border-gray-700 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:outline-none transition-all duration-300"
              ...
              <div className="group">
                ...
                <div className="relative">
                  <FiLock className="absolute left-4 top-3.5 text-gray-500 transition-colors group-focus-within:text-purple-400" />
                  <input
                    type={showPassword ? "text" : "password"}
                    name="password"
                    value={formData.password}
                    onChange={handleChange}
                    className="w-full bg-gray-900/50 text-white rounded-lg pl-12 pr-12 py-3 border border-gray-700 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:outline-none transition-all duration-300"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
              ...
              <div className="flex items-center justify-between">
                ...
                <Link
                  to="/forgot-password"
                  className="text-gray-400 hover:text-purple-400 transition-colors text-sm group relative"
                >
                  Forgot password?
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-4 rounded-lg font-semibold hover:shadow-[0_0_30px_rgba(147,51,234,0.5)] transition-all duration-300 relative group overflow-hidden mt-8 disabled:opacity-70 disabled:cursor-not-allowed"
              >
